...
function Input_Digit(digit){
    ...
    //we are checking to see if Wait_Second_Operand is rue and set Display_Value to the key that was clicked.
...
function Input_Decimal(dot){
    ...
    if (!Calculator.Display_Value.includes(dot)){
        //we are saying that if the Display_Value does nt contain a decimal point, we want to add a decimal point
...
function Handle_Operator(Next_Operator){
    ...
    //and then store the results in Calculator_First_Operand if it doesn't alrady exist
...
keys.addEventListener('click', (event)=> {
    //the target variable is an jobect that represents the element that was clicked
    const {target} = event;
